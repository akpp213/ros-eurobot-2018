...
EPSILON = 0.000000001
MAX_RANGE_EPSILON = 0.1

last_print = time.time()


def rate_limited_print(strg, rate=5.0):
    global last_print
...
class EmpericalModel(MotionModel):
    ...
    def __init__(self):
        ...
        self.poly = lambda a: 1.0 / (
                    a * a * self.polynomial_coeffs[0] + a * self.polynomial_coeffs[1] + self.polynomial_coeffs[2])
...
class DirectUpdate(MotionModel):
    ...
    def update(self, state, dtime):
        try:
            ...
            state[:3] = np.array([trans[0], trans[1], yaw])
            return state
        except (tf.LookupException, tf.ConnectivityException, tf.ExtrapolationException):
            rospy.loginfo("Simulator failed to lookup tf2.")
            return state
    ...
    def simulate(self, evt, omap=None):
        with self.lock:
            ...
            if dt > 0:
                ...
                # wrap the angle difference between -pi and pi
                while deltas[2] > np.pi:
                    deltas[2] -= 2 * np.pi
                while deltas[2] < -np.pi:
                    deltas[2] += 2 * np.pi
...
class Laser(object):
    ...
    def __init__(self, num_rays=None, min_angle=None, max_angle=None, max_range=None, range_method=None, frame=None,
                 omap=None):
        self.max_range = max_range

        self.laser_ranges = np.zeros(num_rays, dtype=np.float32)
        self.expected_ranges = np.zeros(num_rays, dtype=np.float32)
        ...
        self.method = range_method
        self.range_method = None
        self.empty_range_method = None
        self.init_with_other_bot = True
        self.MAX_RANGE_PX = np.ceil(self.max_range / map_info.resolution)
    ...
    def simulate(self, state):
        ...
        self.empty_range_method.calc_range_many(self.queries, self.expected_ranges)

    def update_omap(self, omap):
        map_msg = omap.map_msg
        oMap = range_libc.PyOMap(map_msg)
        # print "Updating map for:", self.method
        if not self.init_with_other_bot:
            if self.method == "bl":
                self.empty_range_method = range_libc.PyBresenhamsLine(oMap, self.MAX_RANGE_PX)
            elif "cddt" in self.method:
                self.empty_range_method = range_libc.PyCDDTCast(oMap, self.MAX_RANGE_PX, self.THETA_DISCRETIZATION)
                if self.method == "pcddt":
                    print "Pruning..."
                    self.empty_range_method.prune()
            elif self.method == "rm":
                self.empty_range_method = range_libc.PyRayMarching(oMap, self.MAX_RANGE_PX)
            elif self.method == "rmgpu":
                self.empty_range_method = range_libc.PyRayMarchingGPU(oMap, self.MAX_RANGE_PX)
            elif self.method == "glt":
                self.empty_range_method = range_libc.PyGiantLUTCast(oMap, self.MAX_RANGE_PX, self.THETA_DISCRETIZATION)
            self.init_with_other_bot = True
            print "Added our other robot to expected map"
        ...
        if self.method == "bl":
            ...
            self.range_method = range_libc.PyGiantLUTCast(oMap, self.MAX_RANGE_PX, self.THETA_DISCRETIZATION)
        # print "Simulated Laser Map Updated..."
...
class Simulator(object):
    def __init__(self):
        ...
        if self.simulate_laser:
            ...
            self.laser_offset = rospy.get_param("~laser_angle_offset")
            self.laser_min_angle = rospy.get_param("~laser_min_angle") + self.laser_offset
            self.laser_max_angle = rospy.get_param("~laser_max_angle") + self.laser_offset
            self.laser_max_range = rospy.get_param("~laser_max_range")
            self.laser_range_method = rospy.get_param("~laser_range_method")
            self.laser_frame = rospy.get_param("~robot_name")
            ...
            self.scan_msg.range_max = self.laser_max_range - 0.1
        ...
        if self.listen_tools:
            ...
            self.rotate_sub = rospy.Subscriber("/headless_sim_rviz/rotate", String, self._rotate_callback,
                                               queue_size=10)
    ...
    def find_robot_coords(self, name):
        try:
            (trans, rot) = self.car.motion_model.listener.lookupTransform('/map', '/' + name, rospy.Time(0))
            yaw = tf.transformations.euler_from_quaternion(rot)[2]
            state = np.array([trans[0], trans[1], yaw])
            return state
        except (tf.LookupException, tf.ConnectivityException, tf.ExtrapolationException):
            rospy.loginfo("Simulator failed to lookup tf2 for " + name)
            return None
    ...
    def _simulate_laser(self, evt):
        ...
        if self.update_omap():
            self.laser.update_omap(self.omap)

        self.scan_msg.header.stamp = stamp
        # self.scan_msg.intensities = np.arange(len(self.scan_msg.ranges))
        self.scan_msg.intensities = np.ones(len(self.scan_msg.ranges)) * 1000
        self.scan_msg.intensities[np.absolute(self.scan_msg.ranges - self.laser.expected_ranges) > self.laser.EPSILON] *= 4
    ...
    def _publish_tf(self, evt):
        ...
        map_laser_pos -= np.dot(
            tf.transformations.quaternion_matrix(tf.transformations.unit_vector(map_laser_rotation))[:3, :3],
...
def make_flamegraph(filterx=None):
    import flamegraph
    perf_log_path = os.path.join(os.path.dirname(__file__),
