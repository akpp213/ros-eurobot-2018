...
def laser_to_rear_wheels(pose, offset=0.005):
    return pose - (offset*np.cos(pose[2]), offset*np.sin(pose[2]), 0.0)


# converts from the given laser pose to the point between the rear tires
# offset is the distance between the laser and rear tires
def rear_wheels_to_laser(pose, offset=0.005):
    return pose + (offset*np.cos(pose[2]), offset*np.sin(pose[2]), 0.0)


# def rear_wheels_to_laser(pose, x_offset=-0.025, y_offset=0.036, th_offset=0):
#     return pose + (x_offset*np.cos(pose[2]) - y_offset*np.sin(pose[2]),
#                    y_offset*np.cos(pose[2]) + x_offset*np.sin(pose[2]), th_offset)
...
class Map(object):
    """ Convenience wrapper for an occupancy grid map object.
        Provides methods to:
            ...
            - dilate the map
    """
    def __init__(self, map_msg, num_other_robots=1):
        self.map_msg = map_msg
        self.map_info = map_msg.info
        self.x0 = map_msg.info.origin.position.x
        self.y0 = map_msg.info.origin.position.y

        self.cspace = None
        self.old_robots = np.array([None]*num_other_robots)
    ...
    def our_robot(self, size, coords):
        # 'occupy' all cells
        robot = np.full(self.occupancy_grid.shape, True, dtype='bool')
    ...
    def update_omap(self, mask, which_robot=0):
        if self.old_robots[which_robot] is not None:
            self.occupancy_grid[self.old_robots[which_robot]] = 1
        self.occupancy_grid[mask] = 0
        self.old_robots[which_robot] = mask
        # plt.imshow(self.occupancy_grid)
        # plt.show()
        self.map_msg.data = (self.occupancy_grid.reshape(self.map_info.height * self.map_info.width) - 1) * -100
