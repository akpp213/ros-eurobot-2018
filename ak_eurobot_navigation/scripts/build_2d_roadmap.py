...
class BuildRoadmap:
    def __init__(self):
        ...
        self.MAX_ANGULAR_RATE = rospy.get_param("~MAX_ANGULAR_RATE", 1)
    ...
    def make_roadmap(self):
        x, y = np.meshgrid(np.arange(self.x0, self.x0 + self.map_width, self.GRID_RESOLUTION),
                               np.arange(self.y0, self.y0 + self.map_height, self.GRID_RESOLUTION))#,
                               # np.arange(-np.pi, np.pi, self.ANGLE_RESOLUTION))
        # self.nodes = np.stack((x.flatten(), y.flatten(), th.flatten()), axis=-1)
        self.nodes = np.stack((x.flatten(), y.flatten()), axis=-1)
        self.nodes = self.nodes[self.configuration_space[0][
            tuple(np.flip(np.moveaxis(self.convert(self.nodes), -1, 0), 0))]]
        ...
        #     # plt.pause(0.0001)
        # plt.show()
        # a=plt.imshow(self.configuration_space[0])
        # for node in self.nodes:
        #     plt.plot(int((node[0] - self.x0)/self.resolution), int((node[1] - self.y0) / self.resolution), 'og')
        # plt.show()
        # self.nodes = [start]
    ...
    def convert(self, nodes):
        ...
        # new[:, 2] = self.closest_angle(np.repeat(new[:, 2, np.newaxis], self.angles.shape[0], axis=1))
    ...
    def find_near_nodes(self, new):
        ...
        near_idxs = np.nonzero((dists != 0) & (dists <= r))[0]# &
    ...
    def line_collision(self, start, end, plot=False):
        ...
        dth = 0#self.theta_diff(start[2], end[2])
        ...
        for i in xrange(len(zero_ind)):
            vals = np.ceil([clip(zero_ind[i] - offsets[order[0]] / self.resolution, order[0]),
                            clip(one_ind[i] - offsets[order[1]] / self.resolution, order[1])]).astype(int)
            ...
            if not self.configuration_space[0][vals[np.where(order == 1)[0][0]],
                                               vals[np.where(order == 0)[0][0]]]:
    ...
    def visualize_roadmap(self):
        for i in xrange(self.edge_ind):
            ...
            # else:
            z = 0#np.array([self.closest_angle(start[2]), self.closest_angle(end[2])])
